...
class Reflector:
    ...
    def reflect(self, goal: str, memory_render: str, recent_history: list[str],
                 trigger: str = '连续失败') -> str:
        """分析最近的卡顿/失败并输出策略建议；失败时返回空串（不阻塞）。

        trigger 说明触发原因：
        - 连续失败：动作本身失败
        - 低进展：动作成功但界面没有实质变化（死循环）
        """
        if self.llm is None:
            return ""
        try:
            ...
            prompt = (
                ...
                f'上面的操作出现了「{trigger}」：请分析原因，并给出下一步策略建议。{chr(10)}'
