...
- 打开具体网站时优先输入完整 URL（https://...）——URL 输入后可以自动验证
  导航是否成功。
- 等待界面出现特定内容（如「结算」按钮、搜索结果）时，用
  wait_for(text, duration_s) 条件等待——它会轮询界面直到目标出现，
  比盲目 wait 更可靠。
- 遇到需要人工介入的场景（手动登录、扫码、验证码等），用 ask_user 动作
  向用户提问，等待回答后再继续。
- 遇到登录弹窗/验证码/扫码等需要用户身份的界面时，**不要反复尝试关闭或
  绕过**（最多尝试一次），直接用 ask_user 请用户处理，等待回答后再继续。
- 涉及支付、下单、发送、提交等动作会被系统拦截并询问用户确认——
  这是正常的，不要试图绕过或改用其他方式规避。
"""
